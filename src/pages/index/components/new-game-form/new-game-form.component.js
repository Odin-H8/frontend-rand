...
module.exports = {
	...
	onKeyPress(e) {
		switch (e.key) {
			...
			case '*': {
				...
				if (this.state.options.game_type === types.X01 || this.state.options.game_type === types.X01HANDICAP || this.state.options.game_type === types.RANDOMIZER) {
	...
	onGameTypeChanged(attribute, value) {
		if (attribute == 'game_type') {
			...
			if (this.state.options.game_type === types.X01 || this.state.options.game_type === types.RANDOMIZER) {
	...
	seedChange(event) {
		this.state.options.seed = event.target.value;
		this.setStateDirty('optional');
	},
